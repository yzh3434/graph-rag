...
|---|---|---|---|---|
| MRR@10 | 0.628 | 0.727 | **0.898** | **+43%** |
| 路由准确率 | 0.740 | 1.000 | **1.000** | +35% |
| Hit@5 | 0.890 | 0.990 | **0.990** | +11% |
| P50 延迟 | 9.86 s | 5.65 s | **5.74 s** | **-42%** |
...
| 检索 | MRR@10 | 0.628 | 0.727 | **0.898** |
| 路由 | Routing Accuracy | 0.740 | 1.000 | **1.000** |
| 生成 | Faithfulness | 0.671 | **0.734** | 0.680 |
| 生成 | Answer Relevancy | 0.900 | 0.902 | **0.907** |
| 系统 | Latency P50 (ms) | 9856 | **5647** | 5742 |
| 系统 | Latency P95 (ms) | 15969 | 12983 | **12351** |
...
**关键观察**：BM25 + RRF 仅作用于 `hybrid_traditional` 路径上的 5 类问题（事实查询 / 属性查询 / 步骤型 / 因果推理 / 实体关系），这 5 类 MRR@10 全部大幅提升（+0.20 ~ +0.32）；走 `graph_rag` 的多跳查询、走 `combined` 的两菜对比完全不受 BM25/RRF 改动影响 —— 验证了 **智能路由模块** 与 **BM25/RRF 融合** 是两个正交独立的改动。
