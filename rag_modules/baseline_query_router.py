"""
baseline 原始纯 LLM 路由器（datawhale 项目初版）
复原自 initial commit 5e45482，用于消融实验对照组。
路由策略：LLM 分析优先，分析失败时规则兜底。

与 IntelligentQueryRouter（改进版）的主要区别：
- 无规则短路层（_pattern_based_route / 实体词典），每次均调用 LLM
- LLM prompt 不含 few-shot 示例，分析维度以 complexity/relation_intensity 为主
"""
...
from rag_modules.intelligent_query_router import SearchStrategy, QueryAnalysis

logger = logging.getLogger(__name__)


class BaselineLLMRouter:
    """
    baseline 纯 LLM 查询路由器（datawhale 项目初版，消融实验对照组）

    路由逻辑：每次查询均调用 LLM 做 complexity/relation_intensity 评估，
    失败时退化到 _rule_based_analysis 规则兜底。无规则短路层，无实体词典预加载。
    """
